		...
		while ($row = mysql_fetch_array($result)){ # los recorro
			...
			if (($row["posicion"] + 1) > $row["cantidad"]){ 
				...
				if( mysql_num_rows($result_p)!=0 ){ 
					...
					WHERE id_lista_tablas = $id_lista_tablas AND posicion = 1 AND id_seccion = ".$row_p["id"];
					...
					if($CHECK===0){
						$mirar_acciones .= "<BR>!!! Corto el proceso 
						por que se lleno un faltante !!!<BR> ";
						break;
					}
	  ...
	  <TD class="Celeste" style="border-bottom: 1px #7B92AD solid; border-left: 1px #7B92AD solid"> 
				<TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
					<TR> 
						...
						<TD>
							<INPUT name="sel_ubi" type="button"  class="bot" onClick="abrirVentana('especiales/ow.php?tabla=estructura&include=estructura&usuario_admin=<? echo $usuario_admin ?>&seleccionar=si&path=../','ubi','450','350')" value="Seleccionar" style="width:100%">
						</TD>
						<? if($tabla=="noticias"){ ?><TD><INPUT name="ver_huecos" type="button"  class="bot" onClick="abrirVentana('especiales/verificador.php?id_estructura='+document.forms[0].id_estructura.value,'ver','450','350')" value="Verificar" style="width:100%"></TD><? } ?>
					</TR>
				</TABLE>
